<template>
  <header
    class="z-50 w-full shadow bg-white dark:bg-gray-900 text-gray-900 dark:text-white 
            flex items-center justify-between px-6 py-3 transition fixed top-0 left-0 right-0"
  >
    ...
    <div class="flex-1 px-10">
      <input
        ...
        class="w-full border rounded-xl px-4 py-2 shadow-sm bg-white 
               dark:bg-gray-800 dark:border-gray-700 dark:text-white 
               focus:ring focus:ring-teal-300 transition"
    ...
    <div class="flex items-center gap-5 mr-6">
      <div class="relative z-50">
        <button
          ...
          class="flex items-center gap-1 hover:text-teal-500 transition"
        >
        ...
        <div
          ...
          class="absolute right-0 mt-2 w-28 bg-white dark:bg-gray-800 
                 border dark:border-gray-700 shadow rounded-lg py-2 z-50"
        >
          <button class="w-full text-left px-3 py-1 hover:bg-gray-100 dark:hover:bg-gray-700"
            @click="setLanguage('id')">Indonesia</button>
          <button class="w-full text-left px-3 py-1 hover:bg-gray-100 dark:hover:bg-gray-700"
            @click="setLanguage('en')">English</button>
          <button class="w-full text-left px-3 py-1 hover:bg-gray-100 dark:hover:bg-gray-700"
            @click="setLanguage('jp')">日本語</button>
        </div>
      </div>
    ...
    <div v-if="!user" class="flex gap-4">
      ...
      <RouterLink
        to="/register"
        class="bg-teal-600 text-white px-4 py-1 rounded-xl hover:bg-teal-700"
      >
    ...
    <div v-else class="flex items-center gap-6">
      ...
      <RouterLink
        to="/cart"
        class="relative transition hover:scale-110 hover:text-teal-600 cursor-pointer"
      >
      ...
      <RouterLink
        to="/wishlist"
        class="relative transition hover:scale-110 hover:text-red-600 cursor-pointer"
      >
        <HeartIcon class="h-7 w-7" />
        <span 
          v-if="store.wishlist && store.wishlist.length > 0" 
          class="absolute -top-2 -right-2 bg-red-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full min-w-[18px] text-center shadow-sm"
        >
      ...
      <div class="relative z-50">
        <img
          :src="currentPhoto"
          class="w-10 h-10 rounded-full cursor-pointer border transition hover:scale-105 object-cover"
          @click="toggleProfileMenu"
        />

        <div
          v-if="profileMenuOpen"
          class="absolute right-0 mt-2 bg-white dark:bg-gray-800 
                 shadow border dark:border-gray-700 rounded-lg w-48 py-2 z-50"
        >
          <RouterLink
            to="/profile"
